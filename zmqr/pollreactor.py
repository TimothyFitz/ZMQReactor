...
class PollReactor(posixbase.PosixReactorBase):
    ...
    pollerFactory = getattr(select, 'poll', None)
    constants = select
    ...
    def __init__(self):
        ...
        the base class.
        """
        self.constants.POLL_DISCONNECTED = (self.constants.POLLHUP | self.constants.POLLERR | self.constants.POLLNVAL)
        self._poller = self.pollerFactory()
        self._selectables = {}
        self._reads = {}
        self._writes = {}
    ...
    def _updateRegistration(self, fd):
        ...
        if fd in self._reads:
            mask = mask | self.constants.POLLIN
        if fd in self._writes:
            mask = mask | self.constants.POLLOUT
        if mask != 0:
    ...
    def _doReadOrWrite(self, selectable, fd, event):
        why = None
        inRead = False
        if event & self.constants.POLL_DISCONNECTED and not (event & self.constants.POLLIN):
            if fd in self._reads:
            ...
            try:
                if event & self.constants.POLLIN:
                    why = selectable.doRead()
                    inRead = True
                if not why and event & self.constants.POLLOUT:
